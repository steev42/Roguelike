using Assets.Scripts.Model.Data.Constants;
...
public class WeaponDefinition
{
    ...
    public bool DoAttack(IAttackableObject attacker, IAttackableObject defender)
    {
        ...
        float hitModLog = Mathf.Log(hitModRatio, Parameters.HIT_DIMINISHMENT_FACTOR);
        float hitChance;

        if (hitModLog >= 0)
        {
            hitChance = 1 - ((1 - Parameters.BASE_HIT_CHANCE) / (1 + hitModLog));
            Debug.Log("log > 0; hit chance = " + hitChance);
        }
        else
        {
            hitChance = (Parameters.BASE_HIT_CHANCE) / (1 - hitModLog);
            Debug.Log("log < 0; hit chance = " + hitChance);
        }
    ...
    public Dictionary<DamageTypes, int> DoDamage(IAttackableObject attacker, IAttackableObject defender)
    {
        ...
        foreach (DamageTypes dt in baseDamage.Keys)
        {
            ...
            if (damageOptimums.ContainsKey(dt))
            {
                ...
                {
                    damageMultiplier = 1 + Mathf.Log(damageStatTotal, Parameters.DAMAGE_DIMINISHMENT_FACTOR);
                    Debug.Log("Damage Multiplier: " + damageMultiplier);
                }
                else
                {
                    damageDivisor = 1 + Mathf.Log(1 / damageStatTotal, Parameters.DAMAGE_DIMINISHMENT_FACTOR);
